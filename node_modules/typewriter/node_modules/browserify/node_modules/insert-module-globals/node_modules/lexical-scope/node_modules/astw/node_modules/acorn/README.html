...
<h3>Main parser</h3><p>This is implemented in <code>dist/acorn.js</code>, and is what you get when you
...
either <code>&quot;script&quot;</code> or <code>&quot;module&quot;</code>.</p>
</li>
...
an error. Defaults to <code>true</code>. When given the value <code>&quot;never&quot;</code>,
...
<pre><code class="language-javascript">for (let token of acorn.tokenize(str)) {
...
<h4>Note on using with <a href="https://github.com/Constellation/escodegen">Escodegen</a></h4><p>Escodegen supports generating comments from AST, attached in
Esprima-specific format. In order to simulate same format in
Acorn, consider following example:</p>
<pre><code class="language-javascript">var comments = [], tokens = [];
...
<h4>Using Acorn in an environment with a Content Security Policy</h4><p>Some contexts, such as Chrome Web Apps, disallow run-time code evaluation.
...
<p>The <code>dist/acorn_csp.js</code> file in the distribution (which is built
by the <code>bin/without_eval</code> script) has the generated code inlined, and
can thus run without evaluating anything.</p>
<h3>dist/acorn_loose.js</h3><p>This file implements an error-tolerant parser. It exposes a single
function.</p>
...
tokenizer.</p>
<h3>dist/walk.js</h3><p>Implements an abstract syntax tree walker. Will store its interface in
...
<h2>Command line interface</h2><p>The <code>bin/acorn</code> utility can be used to parse a file from the command
...
<h2>Build system</h2><p>Acorn is written in ECMAScript 6, as a set of small modules, in the
...
<pre><code>require(&quot;babelify/node_modules/babel-core/register&quot;)
</code></pre><p>That will allow you to directly <code>require</code> the ES6 modules.</p>
<h2>Plugins</h2><p>Acorn is designed support allow plugins which, within reasonable
...
<code>plugin</code> option can be passed, holding an object mapping plugin names
...
<pre><code class="language-javascript">parser.extend(&quot;readToken&quot;, function(nextMethod) {
...
<p>There is a proof-of-concept JSX plugin in the <a href="https://github.com/marijnh/acorn/tree/jsx"><code>jsx</code>
branch</a> branch of the
Github repository.</p></body></html>